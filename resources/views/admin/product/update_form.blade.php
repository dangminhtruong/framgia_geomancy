...
<div class="container-fluid">
    ...
    <form method="POST" action="{{ route('product-save') }}" enctype="multipart/form-data">
        ...
        @include('admin.product.product_category', ['id' => $product->categories_id])
                ...
                <div class="panel panel-filled">
                    ...
                    <div class="panel-body">
                        <div class="form-group">
                            ...
                            <input value="{{ $product->name }}" name="name" type="text" class="form-control" id="exampleInputName" placeholder="{{ __('Tên sản phẩm') }}...">
                            <p class="help-block">{{ $errors->first('name') }}</p>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">{{ __('Giá tiền') }}</label>
                            <input value="{{ str_replace(',', '', $product->price) }}" name="price" type="text" class="form-control" id="exampleInputEmail1" placeholder="{{ __('Giá tiền') }}...">
                            <p class="help-block">{{ $errors->first('price') }}</p>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputPassword1">{{ __('Số lượng') }}</label>
                            <input value="{{ $product->stock }}" name="stock" type="text" class="form-control" id="exampleInputPassword1" placeholder="{{ __('Giá tiền') }}...">
                        ...
                        <div class="attribute-content form-group">
                            ...
                            @foreach ($product->attribute as $key => $value)
                                @if ($key != 'Image' && $key != 'Description')
                                    <div class="box-attr">
                                        <div class="form-group">
                                            <input value="{{ str_replace('_', ' ', $key) }}" class="form-control input-sm m-t-sm width-30 _attr_name" type="text" placeholder="{{ __('Tên thuộc tính') }}">
                                        </div>
                                        <div class="form-inline">
                                            <div class="form-inline zero-left-padding">
                                                <div class="form-group col-md-8 zero-left-padding">
                                                    <input name="{{ str_replace('_', ' ', $key) }}" value="{{ $value }}" type="text" class="form-control full-width" placeholder="{{ __('Giá trị') }}">
                                                </div>
                                                <div class="form-group">
